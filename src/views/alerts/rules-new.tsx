...
const AlertRulesPage: React.FC = () => {
	...
	const columns = [
		...
		},
		{
			...
			dataIndex: "type",
			key: "type",
			render: (type: keyof typeof ALERT_TYPE_MAP) => (
				<Tag color={ALERT_TYPE_MAP[type].color}>{ALERT_TYPE_MAP[type].text}</Tag>
			),
		},
		{
			title: "阈值条件",
		...
		},
		{
			...
			render: (status: string, record: AlertRule) => (
				<Switch
					...
					checkedChildren={ALERT_STATUS_OPTIONS[0].label}
					unCheckedChildren={ALERT_STATUS_OPTIONS[1].label}
				/>
			),
		},
		{
